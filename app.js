...
async function handleFile(e, mode) {
  try {
    ...
    const rows = json.map(r => ({
      ...
      cableOperables: normalizeNumber(r["Cable Operables CI"])
    }));
...
function renderDashboard() {
  ...
  const rows = getFilteredData().map(r => {
    ...
    const liquidezTotal = r.cv7000 + pesosOperablesUsd + r.mepOperables + r.cableOperables;
    const roa = calcRoa(r);
    return {
      ...r,
      pesosOperablesUsd,
...
function renderLiquidezTable(rows, topN) {
  ...
  renderTable("tablaLiquidez", top, row => `
    <tr>
      ...
      <td>${formatMonto(row.cableOperables)}</td>
