...
function renderArancelSummary(rows) {
  const map = new Map();

  rows.forEach(r => {
    const key = r.arancel || "SIN ARANCEL";
    if (!map.has(key)) {
      map.set(key, { arancel: key, clientes: 0, aum: 0, comision: 0 });
    }
    const item = map.get(key);
    item.clientes += 1;
    item.aum += r.aum;
    item.comision += r.comision180;
  });

  const list = [...map.values()].sort((a,b) => b.aum - a.aum);

  renderTable("tablaAranceles", list, row => `
    <tr>
      <td>${escapeHtml(row.arancel)}</td>
      <td>${formatInt(row.clientes)}</td>
      <td>${formatUsd(row.aum)}</td>
      <td>${formatUsd(row.comision)}</td>
    </tr>
  `);

  renderBarChart("chartArancel", {
    labels: list.map(x => x.arancel),
    datasets: [
      { label: "Clientes", data: list.map(x => x.clientes), backgroundColor: "#1f4ea3" },
      { label: "AUM USD", data: list.map(x => x.aum), backgroundColor: "#8eb4ff" }
    ]
  }, { indexAxis: "y" });

  const jCount = rows.filter(r => r.esJuridica === "1").length;
  const njCount = rows.filter(r => r.esJuridica === "0").length;

  renderDoughnutChart("chartJuridica", {
    labels: ["Jurídicas", "No jurídicas"],
    datasets: [{
      data: [jCount, njCount],
      backgroundColor: ["#1f4ea3", "#86a8dd"]
    }]
  });
}
